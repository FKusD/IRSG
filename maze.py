...
# Основано на рабочем `zalup/square.py`: те же пакеты UART + та же одометрия.
...
# Калибровка под лабиринт
CELL_M = 1.0            # одна клетка ≈ 1 метр (поправь, если надо)

# Скорости
TARGET_LINEAR_SPEED = 0.10
DRIVE_OMEGA = TARGET_LINEAR_SPEED / WHEEL_RADIUS
TURN_MOTOR_OMEGA = 4.0
...
def send_motor_command(ser, left_omega, right_omega):
    buf = struct.pack("<ff", float(left_omega), float(right_omega))
    c = crc8()
    c.update(buf)
...
def fwd_cells(ser, n: int):
    if n == 0:
        return
    print(f"Вперёд: {n} клет(ки) (~{n * CELL_M:.2f} м)")
    move_straight_by_position(ser, n * CELL_M, DRIVE_OMEGA)


def left_90(ser):
    print("Поворот: влево 90°")
    turn_in_place_by_position(ser, +math.pi / 2.0, TURN_MOTOR_OMEGA)


def right_90(ser):
    print("Поворот: вправо 90°")
    turn_in_place_by_position(ser, -math.pi / 2.0, TURN_MOTOR_OMEGA)


def drive_maze_hardcoded(ser):
    """
    Хардкод маршрута под лабиринт с картинки (4×4).

    ВАЖНО: это «скрипт» из шагов (вперёд на N клеток / поворот 90°).
    Если на вашем стенде какие-то стены стоят иначе — правьте список действий ниже.

    Старт: левый нижний угол, направление "вперёд" — вправо по нижней стороне.
    Финиш: правый верхний угол.
    """
    ...
    t0 = time.time()
    for i, (cmd, arg) in enumerate(actions, start=1):
        print(f"\nШаг {i}/{len(actions)}: {cmd} {'' if arg is None else arg}")
        if cmd == "F":
            fwd_cells(ser, int(arg))
        elif cmd == "L":
            left_90(ser)
        elif cmd == "R":
            right_90(ser)
    ...
    print(f"\nМаршрут завершён за {time.time() - t0:.1f} c")
